...
const search = new Search(),
  ...
  iphoneTitle =
    "Мобільний телефон Apple iPhone 11 Pro Max 512GB Midnight Green Офіційна гарантія",
  goldIphoneTitle =
    "Мобільний телефон Apple iPhone 11 Pro Max 512GB Gold Офіційна гарантія",
...
describe("Search for phone", () => {
  ...
  it("Find exact iphone and click", () => {
    search.findAndClick();
    expect(getElement({ element: search.productTittle })).toHaveText(
      iphoneTitle
    );
  });
});
...
describe("Verify if iphone is available", () => {
  it("Check if phone is In stock ", () => {
    expect(getElement({ element: search.inStockProduct })).toHaveText(
      inStockPhone
    );
  });
  ...
  it("Check if product seller is Rozetka", () => {
    expect(
      getElement({ element: search.productSeller, needScrollPage: true })
    ).toHaveText(seller);
  });

  it("Check if Guarantee 12 month is available", () => {
    expect(
      getElement({ element: search.guaranteeProduct, needScrollPage: true })
    ).toHaveTextContaining(guaranteeText);
  });
});
